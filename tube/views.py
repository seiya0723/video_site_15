...
from .models import Video,Category,VideoComment,VideoCommentReply,MyList,History,GoodVideo

from users.models import CustomUser,FollowUser,BlockUser


from .serializer import VideoSerializer,VideoEditSerializer,VideoCommentSerializer,VideoCommentReplySerializer,MyListSerializer,HistorySerializer,RateSerializer,GoodSerializer,IconSerializer,FollowUserSerializer,BlockUserSerializer,UserInformationSerializer
...
class SingleView(views.APIView):
    ...
    def get(self,request, video_pk, *args, **kwargs):
        ...
        #HACK:ここはアノテートするのではなくコメント、良いね、マイリストの長さを返すと良い
        #TIPS:.filter()でいまアクセスしている動画で絞り込み、.counts()を使えばヒットしたレコードの数を計測してくれる。
